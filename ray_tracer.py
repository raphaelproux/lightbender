...
Tries to use the native Rust implementation (_raytracer_rs) for speed.
Falls back to a pure-Python/NumPy implementation when the extension is
not available.
"""
...
# ---------------------------------------------------------------------------
# Try to import the Rust extension built with maturin / PyO3.
# ---------------------------------------------------------------------------
try:
    from _raytracer_rs import (
        ray_curve_intersection as _rs_ray_curve_intersection,
        refract_ray as _rs_refract_ray,
        ray_curve_intersection_batch,  # noqa: F401 – re-exported
        refract_ray_batch,  # noqa: F401 – re-exported
    )
...
def _incidence_angle(d: np.ndarray, segment: np.ndarray) -> float:
    ...
    The normal is obtained by rotating *segment* by 90 degrees.
    The returned value is in [0, pi/2]: 0 means the ray is hitting head-on
    (perpendicular to the surface, i.e. along the normal), pi/2 means the
...
def _py_ray_curve_intersection(
    x: float,
    y: float,
    vx: float,
    vy: float,
    ...
    """Pure-Python: closest intersection between a ray and a polyline curve."""
...
def _py_refract_ray(
    ...
    """Pure-Python: refract a ray through a polyline surface (Snell's law)."""
    hit = _py_ray_curve_intersection(x, y, vx, vy, curve)
    if hit is None:
        return None
...
# ---------------------------------------------------------------------------
# Public API -- delegates to Rust when available, else pure Python.
# ---------------------------------------------------------------------------
...
def ray_curve_intersection(
    ...
    - *angle* is the incidence angle in radians between the ray direction and
      the local surface normal (0 = head-on / perpendicular, pi/2 = grazing /
      parallel).
    ...
    """
    if _USE_RUST:
        return _rs_ray_curve_intersection(
            x, y, vx, vy, np.asarray(curve, dtype=np.float64)
        )
    return _py_ray_curve_intersection(x, y, vx, vy, curve)
...
def refract_ray(
    ...
    """
    if _USE_RUST:
        return _rs_refract_ray(
            x, y, vx, vy, np.asarray(curve, dtype=np.float64), n1, n2
        )
    return _py_refract_ray(x, y, vx, vy, curve, n1, n2)
