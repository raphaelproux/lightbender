...
def _(RegularGridInterpolator, np):
    def slice_3d_surface(x, y, z, x0, y0, angle_deg, num_points=100, t_range=10.0):
        ...
        interpolator = RegularGridInterpolator((y[:, 0], x[0, :]), z, bounds_error=False, fill_value=None)
...
@app.cell
def _(mo):
    ast_amp_slider = mo.ui.slider(label='Ast amplitude', start=-0.4, stop=0.4, step=0.05, value=0.0, debounce=False)
    ast_azimuth_slider = mo.ui.slider(label='Ast azimuth', start=-180, stop=180.0, step=1, value=0.0, debounce=False)
    rsi_slider = mo.ui.slider(label='RSI', start=-0.3, stop=0.3, step=0.02, value=0.0, debounce=False)
    power_slider = mo.ui.slider(label='Power', start=-0.5, stop=0.5, step=0.05, value=0.0, debounce=False)
...
def _(
    ...
    _c[3] = power_slider.value+3/4*np.sqrt(5./6)*rsi_slider.value
    ...
    _p = go.Figure()
    _p.add_trace(go.Surface(x=xv, y=yv, z=Phi, contours_z=dict(show=True, usecolormap=True,  highlightcolor="limegreen", project_z=True), showscale=False, cmin=-1.5, cmax=1.5))
    _p.update_layout(
    ...
    _p.update_scenes(camera=dict(
            eye=dict(x=1.5, y=1.5, z=0.5),  # Camera position (higher = farther away)
            up=dict(x=0, y=0, z=1),         # "Up" direction (default: z-axis)
            center=dict(x=0, y=0, z=0)      # Point the camera is looking at
        ),aspectratio={"x": 1, "y": 1, "z": 0.5},camera_projection_type="orthographic")
    ...
    for _angle in _angles:
        _t, _z_slice = slice_3d_surface(xv, yv, Phi, x0=0.0, y0=0.0, angle_deg=_angle, t_range=1)
        _z_slices.append(_z_slice)
        _fig.add_trace(go.Scatter(x=_t, y=_z_slice,
                        mode='lines', line = {"color":'royalblue', "width": 1}))
    _fig.add_trace(go.Scatter(x=_t, y=np.mean(_z_slices, axis=0),
    ...
    _plot_lines = mo.ui.plotly(
      _fig
    )


    _sliders = mo.vstack([ast_amp_slider, ast_azimuth_slider, rsi_slider, power_slider], gap=1)
    mo.vstack([mo.hstack([_plot, _plot_lines], gap=0, align="center"), _sliders], justify="start", align="center", gap=1)
    return (Phi,)


@app.cell
def _(mo):
    slice_x_slider = mo.ui.slider(label='Slice x0', start=-1.0, stop=1.0, step=0.1, value=0.0, debounce=False)
    slice_y_slider = mo.ui.slider(label='Slice y0', start=-1.0, stop=1.0, step=0.1, value=0.0, debounce=False)
    slice_angle_slider = mo.ui.slider(label='Slice angle', start=-180, stop=180, step=0.1, value=0.0, debounce=False)
...
def _(
    ...
    t, z_slice = slice_3d_surface(xv, yv, Phi, x0=slice_x_slider.value, y0=slice_y_slider.value, angle_deg=slice_angle_slider.value, t_range=1)
